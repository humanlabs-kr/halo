...
function Onboarding({ onClose }: OnboardingProps = {}) {
  ...
  const ONBOARDING_STEPS: Array<{
    ...
    {
      title: t("L-yvXhsJRm"),
      subtitle: t("L-LsAm1ggA"),
      heroImage: "/onboarding/step-2.webp",
      cards: [],
    },
    {
      title: t("L-RFg54v2W"),
      subtitle: t("L-0CAAm7Uq"),
      heroImage: "/onboarding/step-2.webp",
      cards: [
        {
          kind: "history",
  ...
  // `currentStep` is always clamped to the slide list by `goToStep`; the
  // fallback keeps the type honest without a cast.
...
function RewardPreviewCard({
  ...
  const { t } = useTranslation();
  const statusConfig = {
    claimable: { label: t("L-shptLOrL"), styles: "bg-black text-white" },
    closed: { label: t("L-bCCe3wJn"), styles: "bg-[#D6D6D6] text-white" },
    claimed: { label: t("L-WtqsPMLa"), styles: "bg-[#D6D6D6] text-white" },
  }[card.status];
